...
void recomByCategory() {
	...
	while (condition) {
		...
		if (scanf("%d", &input) != 1 || input < 0 || input >4) {
			...
			switch (input) {
			case 1:
				selectJoinReview(db, "한식");
				break;
			case 2:
				selectFromRestaurant(db, "양식");
				break;
			case 3:
				selectFromRestaurant(db, "중식");
				break;
			case 4:
				selectFromRestaurant(db, "일식");
				break;
			}
		}
...
selectFromRestaurant(sqlite3* db, const char* category) {
