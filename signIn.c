...
char loggedInId[50] = ""; // 현재 로그인한 사용자의 아이디를 저장하는 변수
int isLoggedIn = 0; // 로그인 상태를 나타내는 변수
...
// 로그인 함수
void signIn(sqlite3* db, Member* member) {
    char id[50], pw[50];
    int rc;
    sqlite3_stmt* stmt;
    ...
    while (!isLoggedIn) {
        ...
        char sql[100];
        sprintf(sql, "SELECT * FROM member WHERE id='%s' AND pw='%s';", id, pw);
